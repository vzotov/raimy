import ClockIcon from '@/components/icons/ClockIcon';
import HourglassIcon from '@/components/icons/HourglassIcon';
import { formatDuration } from '@/lib/duration';
import type { RecipeStep } from '@/types/recipe';

interface StepListProps {
  steps: RecipeStep[];
  onGenerateImage?: (stepIndex: number) => void;
  generatingStepIndex?: number | null;
}

function StepItem({
  step,
  flatIndex,
  displayNumber,
  onGenerateImage,
  generatingStepIndex,
}: {
  step: RecipeStep;
  flatIndex: number;
  displayNumber: number;
  onGenerateImage?: (stepIndex: number) => void;
  generatingStepIndex?: number | null;
}) {
  const isGenerating = generatingStepIndex !== undefined && generatingStepIndex !== null;

  return (
    <li key={step.instruction} className="flex gap-3">
      {step.image_url && (
        <div className="flex-shrink-0 w-32 h-32 sm:w-48 sm:h-48 rounded-xl overflow-hidden">
          <img
            src={step.image_url}
            alt={`Step ${displayNumber}`}
            className="w-full h-full object-cover"
            loading="lazy"
          />
        </div>
      )}
      <span className="flex-shrink-0 w-6 h-6 bg-primary/20 text-primary text-sm font-medium rounded-full flex items-center justify-center">
        {displayNumber}
      </span>
      <div className="flex-1">
        <p className="text-text/80 leading-relaxed">{step.instruction}</p>
        {step.duration && (
          <p className="text-text/60 text-sm mt-1 flex items-center gap-1">
            <ClockIcon className="inline-block w-4 h-4" />{' '}
            {formatDuration(step.duration)}
          </p>
        )}
        {!step.image_url && onGenerateImage && (
          <button
            onClick={() => onGenerateImage(flatIndex)}
            disabled={isGenerating}
            className="mt-2 flex items-center gap-1.5 px-3 py-1.5 text-xs
                       text-text/60 hover:text-text/80 bg-accent/10 hover:bg-accent/20
                       disabled:opacity-50 disabled:cursor-not-allowed
                       rounded-lg transition-colors"
          >
            {generatingStepIndex === flatIndex ? (
              <>
                <HourglassIcon className="w-3.5 h-3.5 animate-spin" />
                Generating...
              </>
            ) : (
              <>
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                Generate image
              </>
            )}
          </button>
        )}
      </div>
    </li>
  );
}

export default function StepList({
  steps,
  onGenerateImage,
  generatingStepIndex,
}: StepListProps) {
  if (steps.length === 0) return null;

  const hasGroups = steps.some((s) => s.group != null);

  if (!hasGroups) {
    return (
      <ol className="space-y-4">
        {steps.map((step, index) => (
          <StepItem
            key={step.instruction}
            step={step}
            flatIndex={index}
            displayNumber={index + 1}
            onGenerateImage={onGenerateImage}
            generatingStepIndex={generatingStepIndex}
          />
        ))}
      </ol>
    );
  }

  const groupOrder: string[] = [];
  const grouped: Record<string, { step: RecipeStep; flatIndex: number }[]> = {};
  const ungrouped: { step: RecipeStep; flatIndex: number }[] = [];

  steps.forEach((step, flatIndex) => {
    if (step.group) {
      if (!grouped[step.group]) {
        groupOrder.push(step.group);
        grouped[step.group] = [];
      }
      grouped[step.group].push({ step, flatIndex });
    } else {
      ungrouped.push({ step, flatIndex });
    }
  });

  // Build render order: grouped sections first, ungrouped last.
  // displayNumber is sequential across all rendered steps (1, 2, 3…) regardless of flat index.
  let displayCounter = 0;

  return (
    <div>
      {groupOrder.map((groupName, i) => (
        <div key={groupName} className={i === 0 ? '' : 'mt-6'}>
          <p className="text-sm font-semibold text-text/60 uppercase tracking-wide mb-3">
            {groupName}
          </p>
          <ol className="space-y-4">
            {grouped[groupName].map(({ step, flatIndex }) => {
              displayCounter += 1;
              return (
                <StepItem
                  key={step.instruction}
                  step={step}
                  flatIndex={flatIndex}
                  displayNumber={displayCounter}
                  onGenerateImage={onGenerateImage}
                  generatingStepIndex={generatingStepIndex}
                />
              );
            })}
          </ol>
        </div>
      ))}
      {ungrouped.length > 0 && (
        <ol className="space-y-4 mt-6">
          {ungrouped.map(({ step, flatIndex }) => {
            displayCounter += 1;
            return (
              <StepItem
                key={step.instruction}
                step={step}
                flatIndex={flatIndex}
                displayNumber={displayCounter}
                onGenerateImage={onGenerateImage}
                generatingStepIndex={generatingStepIndex}
              />
            );
          })}
        </ol>
      )}
    </div>
  );
}
